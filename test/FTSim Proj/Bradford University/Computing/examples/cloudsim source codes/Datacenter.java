...
import southampton.ecs.desktopcloudsim.Calculation;
import southampton.ecs.desktopcloudsim.Constants;
import southampton.ecs.desktopcloudsim.MonitorUnit;
...
public class Datacenter extends SimEntity {
	...
	protected void processVmDestroy(SimEvent ev, boolean ack) {
		...
		System.out.println("[datacenter.processVmDestroy]: destruction of vm# " + vm.getId());
		MonitorUnit.addDestroyedVm(vm);
	}
	...
	protected void processVmMigrate(SimEvent ev, boolean ack) {
		...
		//this can be changed to be the chosen host is made here
		boolean result = getVmAllocationPolicy().allocateHostForVm(vm, host);
		
		if (!result) 
		{
			//scheduleVmMigration(vm);
			System.out.println("[Datacenter.processVmMigrate] VM allocation to the destination failed, system is ABORTED");
			//sendNow(ev.getSource(), CloudSimTags.VM_DESTROY, vm);
			System.exit(0);
		}
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			if (host == null)		//this vm has been destroyed
				vm = getVmForCloudletMigration(vmId, userId);
	...
	protected void processCloudletCancel(int cloudletId, int userId, int vmId) {
		...
		//inform the broker, this cloudlet has finished executing  
		sendNow(userId, CloudSimTags.CLOUDLET_RETURN, cl);
	}
	
	protected void processMonitoringEvent()
	{
		System.out.println(CloudSim.clock() / MonitorUnit.MONITORING_INTERVAL + ": processing monitor event");
		
		List<Host> tmpHostList = getVmAllocationPolicy().getHostList();
		
		MonitorUnit.setSimulationTime(CloudSim.clock());
		MonitorUnit.setCloudletAvgExecTime(Calculation.calculateAverageTime(MonitorUnit.getFinishedCloudletList()));
		MonitorUnit.setCloudletMicroAvgExecTime(Calculation.calculateMicroAverageTime(MonitorUnit.getFinishedCloudletList()));

		MonitorUnit.setUtilisation(Calculation.calculateAverageUtilisation(tmpHostList));		
		MonitorUnit.setAavailability(Calculation.calculateAverageAvailability(tmpHostList));
		
		MonitorUnit.setIdleMachineNumber(Calculation.calculateNumberOfIdleMachines(tmpHostList));

		MonitorUnit.writeEvent();
	}
	...
	protected void processHostFailure(SimEvent ev) {
		...
		MonitorUnit.addMachineFailure();
		//Log.printLine("\t\t\t" + CloudSim.clock() +": ****** host id " + host.getId() + "  failed ****\t\t failure number " + MonitorUnit.getMachineFailure());
				
		for (Vm vm : host.getVmList()) {		
			//Log.printLine("\t\t\t" + CloudSim.clock() +": ****** vm id " + vm.getId() + " is going to destroyed " + MonitorUnit.getMachineFailure());
			scheduleVmDestruction(vm);
			//scheduleVmMigration(vm);
			//scheduleCloudLetPenalty(vm);
		}

		scheduleHostRemoving(host);
	}
	...
	protected void processHostJoining(SimEvent ev){
		...
		//add the new free Pes to the list
		getVmAllocationPolicy().addFeePes(host.getNumberOfPes());
		//Log.printLine("\t\t\t" + CloudSim.clock() +": ****** host id " + host.getId() + " has joined **********\n");
		
		MonitorUnit.removeMachineFailure();
	}
	...
	public void shutdownEntity() {
		...
		/*Log.printLine(getName() + " In datacentre: shutdownEntity starting printing list of available hosts...");
		
		List<? extends Host> hostList = getVmAllocationPolicy().getHostList();
		for(int i=0; i < hostList.size(); i++){
			Log.printLine("host id " + hostList.get(i).getId());
		}*/
		
		//cannot print details cos the vms are already gone :(
		//printDetails();
	...
	{	
		for (Host host: getCharacteristics().getHostList())
		{
			if (host.getId() == hostId)
			{
				...
				if (aliveTime >= 0 && aliveTime <= Constants.SIMULATION_LIMIT)
	...
	{
		//wanted to get list of running cloudlet
		...
		Log.printLine("[datacentre.scheduleCloudLetPenalty] printing cloudlets details for vm #" + vm.getId() );
		...
		for (int i=0; i < cloudletList.size(); i++)
		{
			...
			if (cloudlet.getVmId() == vm.getId() &&  cloudlet.getStatus() == Cloudlet.INEXEC )
			{
				...
				//Log.printLine("cloudlet id " + cloudlet.getCloudletId() + " cloudlet.getStatus() " + cloudlet.getStatus() );
				//sendNow(this.getId(), CloudSimTags.CLOUDLET_CANCEL, cloudlet);
				//broker.bindCloudletToVm(cloudlet.getCloudletId(), vm.getId());

				//restart a cloudlet
				sendNow(this.getId(), CloudSimTags.CLOUDLET_CANCEL, cloudlet);			
				//send(this.getId(), CloudSim.getMinTimeBetweenEvents(), CloudSimTags.CLOUDLET_SUBMIT, cloudlet);/**/
				
				/*send(this.getId(), CloudSim.getMinTimeBetweenEvents(), CloudSimTags.CLOUDLET_PAUSE, cloudlet);				
				double submisionTime = CloudSim.clock() - cloudlet.getExecStartTime();
				send(this.getId(), submisionTime, CloudSimTags.CLOUDLET_RESUME, cloudlet);/***/
				
				//Log.printLine("submisionTime : " + submisionTime);				
			}
		}
	}
	...
	{			
		//Log.printLine("\t *******vm destruction ******\t\t vm " + vm.getId() + " is set to be destroyed, naive asumption ");
		
		//* Cancelling all associated cloudlets
		DatacenterBroker broker = (DatacenterBroker) CloudSim.getEntity(vm.getUserId());
		for(Cloudlet cloudlet: broker.cloudletSubmittedList)
		{
			if (cloudlet.getVmId() == vm.getId())
			{
				if(cloudlet.getStatus() == Cloudlet.INEXEC)
				{
					...
					sendNow(vm.getUserId(), CloudSimTags.CLOUDLET_RETURN, cloudlet);
				}
				else if (cloudlet.getStatus() != Cloudlet.SUCCESS)
				{
					int vmDestId = getVmIdToMigrateCloudlet(vm);
		...
		//inform the broker that there was a vm destruction
		send(this.getId(), 0, CloudSimTags.VM_DESTROY, vm);
	}
	...
	{					
		...
		if (migrateToHostIndex == -1)	//no host can accomodate this vm, we can here destroy the vm
		{
			//setting all associated cloudlets (running or will run) to be cancelled
			sendNow(vm.getUserId(), CloudSimTags.VM_DESTROY, vm);
			
			Log.printLine("*******vm destruction ******\t\t vm " + vm.getId() + " is set to be destroyed, no availability\t datacentre id: " + this.getId());
	...
	protected void scheduleHostRemoving(Host host){
		//sendNow(this.getId(), CloudSimTags.HOST_REMOVE, host);
		send(this.getId(), CloudSim.getMinTimeBetweenEvents(), CloudSimTags.HOST_REMOVE, host);
	}
	
	protected void scheduleMonitoringEvents(double simulationTime)
	{	
		int src = this.getId();	
		for (double timeCounter = 0; timeCounter <= simulationTime; timeCounter = timeCounter + MonitorUnit.MONITORING_INTERVAL)
		{
			CloudSim.send(src , src, timeCounter, CloudSimTags.MONITOR, null);
		}
	}
	
	protected void removeHost(SimEvent ev){
		Host host = (Host) ev.getData();
		List<? extends Host> hostList = getVmAllocationPolicy().getHostList();
		...
		//Log.printLine("\t\t\t" + CloudSim.clock() +": ****** host id " + host.getId() + " has been removed **********\n");
	}
	...
	protected Vm getVmForCloudletMigration(int vmId, int userId)
	{
		int vmIndex = -1;
		Random r = new Random();
		Host host =  null;
		...
		/*
		System.out.println("printing vms details : ");
		
		for(Vm vm : getVmList())
		{
			System.out.print("vm #" + vm.getId() + " utilsation: " + vm.getTotalUtilizationOfCpuMips(CloudSim.clock()));
		}
		System.out.println(); /**/
		
		while(host == null)
		{
			vmIndex = r.nextInt() % vmList.size();
			if (vmIndex < 0)
				vmIndex = vmIndex * -1;
			
			host = getVmAllocationPolicy().getHost(vmIndex, userId);
		}
	...
	public void setMonitoringEvents(double simulationTime)
	{
		this.scheduleMonitoringEvents(simulationTime);
	}
