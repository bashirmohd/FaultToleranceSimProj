...
public class Host {
	...
	{
		init(id, ramProvisioner, bwProvisioner, storage, peList, vmScheduler);
	}
	...
	private void init(int id, RamProvisioner ramProvisioner, BwProvisioner bwProvisioner,
	...
	public void addMigratingInVm(Vm vm) {
		...
		if (!getVmsMigratingIn().contains(vm)) {
			...
			if (vm.getHost() == null)
			{
				...
				//System.out.println("getVmsMigratingIn().isEmpty()");
				System.out.println("gvm.getHost() == null");
	...
	public boolean isSuitableForVm(Vm vm) {
		...
		result = (getVmScheduler().getPeCapacity() >= vm.getCurrentRequestedMaxMips()
		...
		if (getMaxAvailableMips() < vm.getMips()			
				|| getStorage() < vm.getSize()
				|| getRamProvisioner().getAvailableRam() < vm.getRam() 
				|| getBwProvisioner().getAvailableBw() < vm.getBw() )
			result = false;/**/
	...
	public boolean vmCreate(Vm vm) {
		...
		if (!getRamProvisioner().allocateRamForVm(vm, vm.getCurrentRequestedRam())) {
			Log.printLine("[VmScheduler.vmCreate] Allocation of VM #" + vm.getId() + " to Host #" + getId()
					+ " failed by RAM");
			System.out.println("[VmScheduler.vmCreate] Allocation of VM #" + vm.getId() + " to Host #" + getId()
					+ " failed by RAM, RAM = " + getRam() + " getRamProvisioner().getAvailableRam() = " + getRamProvisioner().getAvailableRam());
			ret = false;
		}
		...
		if (!getVmScheduler().allocatePesForVm(vm, vm.getCurrentRequestedMips())) {
			Log.printLine("[VmScheduler.vmCreate] Allocation of VM #" + vm.getId() + " to Host #" + getId()
					+ " failed by MIPS");
			System.out.println("[VmScheduler.vmCreate] Allocation of VM #" + vm.getId() + " vm.getMips(): " + vm.getMips() + " vm.getCurrentRequestedMips(): " + vm.getCurrentRequestedMips()  + " to Host #" + getId()
					+ " failed by MIPS" + " getAvailableMips(): " + getAvailableMips() + " getMaxAvailableMips: " + getMaxAvailableMips() + " getPeList().size(): "+ getPeList().size() 
					+ " getVmScheduler().getPeCapacity(): " + getVmScheduler().getPeCapacity());
		...
		if(ret == false)
		{
			System.out.print("current allocated vms: ");
			for(Vm tmpVM: getVmList())
				System.out.print(" vm #" + tmpVM.getId() + " its required RAM " + tmpVM.getRam());
	...
	//////////
	public double getTotalMIPSforFutureMigrations()
	{
		double totalMIPS = 0;
		for( Vm vm: getVmsMigratingIn())
		{
	...
	public double getAvailableMIPS()
	{
		double totalUsedMIPS = 0, totalMips = 0;
		for( Vm vm: getVmList())
		{
		...
		return totalMips - totalUsedMIPS;
	}
	
	public double getUtilisation()
	{
		return getUsedMIPS()/getTotalMips();
	}
	
	public double getUtilisationPercentage()
	{
		return getUtilisation() * 100;
	}
	
	public double getAvailabilityPercentage()
	{
		
		return (getAvailableMips()/getTotalMips()) * 100;
	}
	
	/**
	 * get utilisation of a core with maximum utilisation 
	 * @return
	 */
	public double getMaxUtilisationInCore(Vm vm)
	{
		double maxUtil = -1;
		for (Pe pe : getPeList()) {
			double tmp = pe.getPeProvisioner().getUtilization();
			if (tmp > maxUtil && pe.getPeProvisioner().getAvailableMips() >= vm.getMips()) {
				maxUtil = tmp;
			}
		}
		return maxUtil * 100;
	...
	public double getMinUtilisationInCore(Vm vm)
	{
		double minUtil = Integer.MAX_VALUE;
		for (Pe pe : getPeList()) {
			double tmp = pe.getPeProvisioner().getUtilization();
			if (tmp < minUtil && pe.getPeProvisioner().getAvailableMips() >= vm.getMips()) {
				minUtil = tmp;
			}
		}
		return minUtil * 100;
	...
	protected boolean suitableHostToMigrate(double mips)
	{
		boolean result = true;
		if ((mips + getTotalMIPSforFutureMigrations()) > getAvailableMips())
			result = false;
		return result;
	}
	...
	public double getUtilizationOfCpu() {
		double utilization = getUtilisation() / getTotalMips();
	...
	public double getPower() {
		return getPower(getUtilisation());
	}
	...
	public void setEnergyConsumption(double e)
	{
		consumedEnergy = e;
	}
