...
public class CloudSim {
	...
	public static void addEntity(SimEntity e) {
		SimEvent evt;
		//Log.printLine("***Entitiy creating " + e.getName() + " ");
		if (running) {
	...
	public static boolean runClockTick() {
		...
		//this loop is to run all entity with its associated events that should happen now
	...
	public static void send(int src, int dest, double delay, int tag, Object data) {
		...
		//Log.printLine("\t\t\t\t CloudSim.send: " + e.toString());
		future.addEvent(e);
	}
	...
	public static double run() {
		//if this is the first time, then call runStart
		if (!running) {
			//printMessage("checking running = " + running );
			runStart();
		}
int itr=0;
		while (true) {

//me it is me : Log.printLine("\t\t\t\t\t" + ++itr + " : time: " + clock() + "\t found it in run method in cloudsim class");			
